...
async function loadOutlets() {
  const { data: outlets, error } = await supabaseClient.from('outlets').select('*').order('name')
...
async function loadAnalytics() {
  ...
  const { data: monthOrders, error: err1 } = await supabaseClient
    .from('orders')
    .select('total_amount')
    .gte('created_at', startOfMonth.toISOString())
  if (err1) throw err1

  const { data: allReviewsData, error: err2 } = await supabaseClient
    .from('reviews')
    .select('rating')
  if (err2) throw err2

  const { data: topItems, error: err3 } = await supabaseClient
    .from('order_items')
